  ...
  const StudentDashboard = () => {
    ...
    const renderCircularChart = (departmentsData) => {
      const approvedCount = departmentsData.filter(department => department.isApproved).length;
      const pendingCount = departmentsData.length - approvedCount;
                  ...
                  <ul className="list-group list-group-flush">
                    {departments.length > 0 && departments.map((department, index) => (
                      <li className="list-group-item d-flex justify-content-between" key={index}>
                        {department.name}
                        <span className={department.isApproved ? "text-success" : "yellow"}>
                          {department.isApproved ? `Approved by ${department.adminName}` : "Pending"}
                        </span>
                      </li>
                    ))}
                  </ul>
